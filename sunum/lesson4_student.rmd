...
ggplot(aes(x=age, y=friendships_initiated), data=pf)+ 
  xlim(13,90)+
  geom_jitter(alpha = 0.05, position = position_jitter(height = 0), color='orange')+
  coord_trans(y='sqrt')+
...
ggplot(data = subset(pf.fc_by_age_months, age_with_months<71), aes(x=age_with_months, y=friend_count_median)) + geom_line()
...
p3 <-ggplot(aes(x=round(age/5)*5, y=friend_count), data=subset(pf, age<71)) +geom_line(stat='summary', fun.y = mean)+geom_smooth()
